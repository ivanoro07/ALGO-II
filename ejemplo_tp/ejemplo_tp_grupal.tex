...
{\existe[unalinea]{lista}{\TLista{\ent}}{(|lista| > 1) \land (|lista|\leq |distancias|)\land\paraTodo[unalinea]{i}{\ent}{1\leq i< |lista| \implicaLuego ((lista[0]=desde)\land(lista[|lista|-1]=hasta) \land \\ \paraTodo[unalinea]{a}{\ent}{0\leq a<|lista| \implicaLuego 0 \leq lista[a] < |distancias|)\yLuego (distancias[lista[i]][lista[i-1]]>0) }}}}
...
\begin{proc}{grandesCiudades}{\In ciudades : \TLista{Ciudad} }{\TLista{Ciudad}}
	...
	\asegura{\paraTodo [unalinea]{j}{\ent}{0 \leq j<|ciudades| \implicaLuego (tieneMasDe50k(ciudades[j])  \land ciudades[j]  \in  res) \lor \\(\neg tieneMasDe50k(ciudades[j])  \land ciudades[j]  \notin  res)}}
...
Dada la matriz de conexión de orden 1, este procedimineto debe obtener aquella de orden n que indica cuántos caminos
...
\hspace{1cm}Implicado en la wp\\
\[res = \sum\limits_{j=0}^{i-1}ciudades[j].habitantes\]
\hspace{1cm}Es identico en I, tanto como en la Wp
\vspace{0.5cm}\\
...
\hspace{-0.55cm}2)$Pc \implicaLuego wp(while..., Qc)$   \\
